...
class Platform {
    ...
    func authenticate(person: User, completion: (databack: NSDictionary?, error: NSError?) -> Void) {
        ...
        var task: NSURLSessionDataTask = NSURLSession.sharedSession().dataTaskWithRequest(request) {
            ...
            println("HI")
            if error != nil {
                completion(databack: nil, error: error)
            } else {
                ...
                completion(databack: readdata, error: error)
            }
